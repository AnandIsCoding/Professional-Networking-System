...
function UserContactsPanel({ setActive, activeConversationId, item }) {
  ...
  return (
    <div
      onClick={() => setActive(item?._id, friend)}
      key={item?._id}
      className={`flex flex-col sm:flex-row sm:gap-2 border-b-1 border-l-0 border-r-0 border-zinc-300 hover:bg-blue-50 p-2 cursor-pointer ${
        activeConversationId === item._id ? "bg-blue-50" : ""
      } `}
    >
      ...
      <div className="flex flex-col sm:pt-1 ">
        ...
        <p className="text-gray-600 text-sm">
          {friend?.headline?.split(" ").slice(0, 5)?.join(" ")}
        </p>
      </div>
    </div>
  );
